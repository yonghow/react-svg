(function (global, factory) {
  ...
  var loadSvg = function loadSvg(url, withCredentials, callback) {
    if (svgCache.has(url)) {
      var cacheValue = svgCache.get(url);
  ...
  var injectElement = function injectElement(el, evalScripts, renumerateIRIElements, beforeEach, withCredentials, callback) {
    ...
    el.setAttribute('src', '');
    loadSvg(imgUrl, withCredentials, function (error, svg) {
      if (!svg) {
        injectedElements.splice(injectedElements.indexOf(el), 1);
        el = null;
        callback(error);
  ...
  var SVGInjector = function SVGInjector(elements, _a) {
    var _b = _a === void 0 ? {} : _a,
      ...
      return undefined;
        ...
        _h = _b.withCredentials,
        withCredentials = _h === void 0 ? false : _h;

    if (elements && 'length' in elements) {
      var elementsLoaded_1 = 0;

      for (var i = 0, j = elements.length; i < j; i++) {
        injectElement(elements[i], evalScripts, renumerateIRIElements, beforeEach, withCredentials, function (error, svg) {
          afterEach(error, svg);
      ...
      injectElement(elements, evalScripts, renumerateIRIElements, beforeEach, withCredentials, function (error, svg) {
        afterEach(error, svg);
        afterAll(1);
        elements = null;
      });
